...
def restore_audio(target_path : str, output_path : str) -> bool:
	...
	# 获取截取帧的起始帧
	trim_frame_start = facefusion.globals.trim_frame_start
	print(f"trim_frame_start = {trim_frame_start}")
	# 获取截取帧的结束帧
	trim_frame_end = facefusion.globals.trim_frame_end
	print(f"trim_frame_end = {trim_frame_end}")
	# 获取临时输出视频路径
	temp_output_video_path = get_temp_output_video_path(target_path)
	print(f"temp_output_video_path = {temp_output_video_path}")
	# 初始化ffmpeg命令
	commands = [ '-hwaccel', 'auto', '-i', temp_output_video_path ]
	# 如果截取帧的起始帧不为空，则添加截取帧的起始帧
	if trim_frame_start is not None:
		start_time = trim_frame_start / fps
		commands.extend([ '-ss', str(start_time) ])
	# 如果截取帧的结束帧不为空，则添加截取帧的结束帧
...
def get_temp_output_video_path(target_path : str) -> str:
	...
	# # TODO
	# return "/mnt/c/Python/facefusion/.assets/examples/temp_1.mp4"
...
def create_temp(target_path : str) -> None:
	# 获取临时文件夹路径
	temp_directory_path = get_temp_directory_path(target_path)
	print(f" 临时文件夹：{temp_directory_path}")
	# 创建临时文件夹，如果存在则不创建
